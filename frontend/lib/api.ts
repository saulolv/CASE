...
const API = useProxy ? "/backend" : process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
...
export type Run = { run_id: string; trigger: string; provider: string; model?: string; latency_ms: number; estimated_cost_usd: number; fallback: boolean; evidence: string[]; created_at: string; decision: { action: string; rationale: string[]; confidence?: number; requires_human_review?: boolean } };
